...
class Grasp_dexterous_object(Task):
    ...
    def penalty_rneg(self):
        d = torch.norm(self.sim.get_dneg(), dim=-1)

        rneg_per_finger = torch.exp(-self.alpha_neg * d)
    ...
    def reward_pos_reach_distance(self):
        ...
        rpos_per_finger = torch.exp(-self.alpha_pos * d)
