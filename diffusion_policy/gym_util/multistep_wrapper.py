...
class MultiStepWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        # observation/action space 不再重复 n_envs
    ...
    def _aggregate_rewards(self):
        ...
        for i in range(self.n_envs):
            if len(self.reward[i]) == 0:
                rewards.append(0.0)
            else:
                if self.reward_agg_method == 'max':
                    rewards.append(np.max(self.reward[i]))
                elif self.reward_agg_method == 'sum':
                    rewards.append(np.sum(self.reward[i]))
                elif self.reward_agg_method == 'mean':
                    rewards.append(np.mean(self.reward[i]))
                else:
                    raise NotImplementedError
